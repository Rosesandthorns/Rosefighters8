...
export default function GameCanvas() {
  ...
  useEffect(() => {
    ...
    newSocket.on('playerEffect', (data: { id: string, effect: string }) => {
    ...
    const updatePhysics = () => {
      ...
      if (safetyWarpReadyRef.current) {
          ...
          if (myPlayer.isGrabbingLedge) {
              ...
              // Jump
              if ((keys['ArrowUp'] || keys['KeyW'] || keys['Space']) && myPlayer.isGrounded && !(myPlayer.activeEffects?.['ricaCharge'] > Date.now())) {
                myPlayer.velocity.y = JUMP_FORCE;
